...
module fpga_signal_control_top (
    ...
    // ToF sample-start line (if want to drive a Pmod SS pin later)
    wire tof_ss_int;

    // VGA sync from core (active-LOW in original core)
    ...
    assign uart_txo_debug = uart_txo_debug_int;
    assign pwm_out        = pwm_out_int;
    assign fan_en_ja4     = fan_en_ja4_int;
    assign tof_ss         = tof_ss_int;

    // If the core doesn't drive LED1, tie off or connect as needed:
    assign led1_r = 1'b0;
    assign led1_g = 1'b0;
    assign led1_b = 1'b0;
    ...
    // ------------------------------------------------------------------------
    // VGA sync polarity adaptation (core uses *_n, board expects active-LOW)
    // ------------------------------------------------------------------------
