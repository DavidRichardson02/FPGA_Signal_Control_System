...
//==============================================================================
    ...
    /*
    // ---------------------------------------------------------------------
    // UART TX: 2,000,000 baud @ 100 MHz, 8-N-1
    // mapper_packetizer drives data_in / data_valid,
    // and observes data_ready (pkt_rdy) for backpressure.
    // ---------------------------------------------------------------------
    uart_tx #(
        ...
        // Ready/valid stream from mapper_packetizer
        .data_in    (pkt_byte),   // 8-bit payload
        .data_valid (pkt_vld),    // 1-cycle strobe when pkt_byte is valid
        .data_ready (pkt_rdy),    // backpressure → mapper_packetizer.tx_rdy
    
        // Status + physical line
        .busy      (/ *   can route to an LED if desired * /),
        .tx         (uart_txo)    // goes to FTDI RXD pin (D4 in XDC)
    );
    
    //==============================================================================
    // UART TX debug: log every byte accepted by uart_tx to the Vivado transcript
    //==============================================================================
    reg [7:0] ch;
    always @(posedge clk_100MHz) begin
        if (!rst && byte_accepted) begin
            // Printable ASCII helper: show '.' for non-printables
            ch = ((pkt_byte >= 8'h20) && (pkt_byte <= 8'h7E)) ? pkt_byte : 8'h2E;
    
            $display("[%0t ns] UART_TX byte: 0x%02h (%0d, '%s')",
                     $time, pkt_byte, pkt_byte, ch);
        end
    end
    //*/
    ///*
    ...
    //----------------------------------------------------------------------------
    // Behavior:
    //   sw_logo_sel=0 => show buffer 0
    //   sw_logo_sel=1 => show buffer 1
    //
    // Implementation strategy (robust):
    //   1) Debounce + synchronize sw_logo_sel into clk_100MHz.
    //   2) Generate a *sticky* manual request when desired != active.
    //      - Sticky request is ideal because the pix-domain 2FF sync cannot "miss"
    //        a level that persists for many cycles.
    //   3) Give manual selection PRIORITY over auto swap requests, so UART-driven
    //      frame-complete swaps can't immediately flip away from forced bank.
    //----------------------------------------------------------------------------
    
    // ---------------------------
    // 1) Sync + debounce switch
    // ---------------------------
    
    // 2-FF synchronizer into clk_100MHz
    ...
    // Simple debounce for a slide switch.
    // Can reuse RST_DB_* if want 5ms; slide switches often need less.
    // Here: 2 ms @ 100 MHz = 200,000 cycles.
    localparam integer LOGO_SW_DB_MS  = 2;
    localparam integer LOGO_SW_DB_MAX = (100_000 * LOGO_SW_DB_MS); // 200,000
    localparam integer LOGO_SW_DB_W   = CLOG2(LOGO_SW_DB_MAX+1);
    
    reg [LOGO_SW_DB_W-1:0] logo_sw_cnt = {LOGO_SW_DB_W{1'b0}};
    reg                    logo_bank_sel = 1'b0;   // debounced desired bank (0/1)
    
    always @(posedge clk_100MHz) begin
        if (rst) begin
            logo_sw_cnt   <= {LOGO_SW_DB_W{1'b0}};
            logo_bank_sel <= 1'b0;
        end else begin
            if (logo_sel_s1 == logo_bank_sel) begin
                logo_sw_cnt <= {LOGO_SW_DB_W{1'b0}};
            end else if (logo_sw_cnt == LOGO_SW_DB_MAX[LOGO_SW_DB_W-1:0]) begin
                logo_bank_sel <= logo_sel_s1; // accept new stable position
                logo_sw_cnt   <= {LOGO_SW_DB_W{1'b0}};
            end else begin
                logo_sw_cnt <= logo_sw_cnt + 1'b1;
            end
        end
    end
    
    // ---------------------------
    // 2) Sticky manual swap request
    // ---------------------------
    // If desired != active, hold request HIGH until active matches desired.
    // This guarantees the pix-domain synchronizer sees it, and ensures exactly
    // one toggle is requested (since there are only 2 banks).
    reg logo_swap_req_manual_sticky = 1'b0;
    
    always @(posedge clk_100MHz) begin
        if (rst) begin
            logo_swap_req_manual_sticky <= 1'b0;
        end else begin
            if (logo_active_buf_sys == logo_bank_sel) begin
                // Achieved desired selection => deassert request
                logo_swap_req_manual_sticky <= 1'b0;
            end else begin
                // Not yet at desired selection => keep request asserted
                logo_swap_req_manual_sticky <= 1'b1;
            end
        end
    end
    
    wire logo_swap_req_manual = logo_swap_req_manual_sticky;
    
    // ---------------------------
    // 3) Combine auto/manual swap requests (manual has priority)
    // ---------------------------
    // If the user is forcing a bank (manual request active), do NOT allow an
    // auto-swap to fight it.
    assign logo_swap_req_sys = logo_swap_req_manual
                             | (logo_swap_req_auto & ~logo_swap_req_manual);
    ...
    // -------------------------------------------------------------------------
    // ACCEL-only overlay output (pix domain)
    // -------------------------------------------------------------------------
    wire [3:0] accel_ovl_r;
    wire [3:0] accel_ovl_g;
    wire [3:0] accel_ovl_b;

    wire [11:0] rgb_overlay_accel = {accel_ovl_r, accel_ovl_g, accel_ovl_b};
    ...
    wire signed [7:0] accel_x_sys;
    wire signed [7:0] accel_y_sys;
    wire signed [7:0] accel_z_sys;
    // 100 MHz system domain
    wire  signed [7:0] accel_x_q7_sys;
    wire  signed [7:0] accel_y_q7_sys;
    wire  signed [7:0] accel_z_q7_sys;
    wire signed [7:0] accel_x_raw_sys;
    wire signed [7:0] accel_y_raw_sys;
    wire signed [7:0] accel_z_raw_sys;
    
    
    
    wire               accel_sample_valid_sys;
    wire               accel_init_done;
    wire               accel_devid_error;
    
    adxl362_if #(
        .PWRUP_WAIT_CYCLES  (32'd1_000_000),
        .SAMPLE_WAIT_CYCLES (32'd1_000_000)
    ) u_adxl362_if (
        .clk          (clk_100MHz),
        .rst          (rst),
        ...
        // Q7-scaled acceleration
        .accel_x      (accel_x_q7_sys),
        .accel_y      (accel_y_q7_sys),
        .accel_z      (accel_z_q7_sys),
        
        // Raw acceleration
        .raw_x        (accel_x_raw_sys),
        .raw_y        (accel_y_raw_sys),
        .raw_z        (accel_z_raw_sys),
        
      
        .sample_valid (accel_sample_valid_sys),
    
        .init_done    (accel_init_done),
        .devid_error  (accel_devid_error),
    ...
    wire signed [7:0] accel_x_q7_pix;
    wire signed [7:0] accel_y_q7_pix;
    wire signed [7:0] accel_z_q7_pix;
    wire signed [7:0] accel_x_raw_pix;
    wire signed [7:0] accel_y_raw_pix;
    wire signed [7:0] accel_z_raw_pix;
    wire              accel_update_pix;

    accel_cdc_bridge u_accel_cdc_bridge (
        .sys_clk                (clk_100MHz),
        .sys_rst                (rst),
        
        .accel_x_q7_sys    (accel_x_q7_sys),
        .accel_y_q7_sys    (accel_y_q7_sys),
        .accel_z_q7_sys    (accel_z_q7_sys),
        
        .accel_x_raw_sys         (accel_x_raw_sys),
        .accel_y_raw_sys         (accel_y_raw_sys),
        .accel_z_raw_sys         (accel_z_raw_sys),
        
        .accel_sample_valid_sys (accel_sample_valid_sys),
        
        .pix_clk                (pix_clk),
        .pix_rst                (rst),          // pixel domain reset = system reset
        .accel_x_q7_pix    (accel_x_q7_pix),
        .accel_y_q7_pix    (accel_y_q7_pix),
        .accel_z_q7_pix    (accel_z_q7_pix),
        
        .accel_x_raw_pix          (accel_x_raw_pix),
        .accel_y_raw_pix          (accel_y_raw_pix),
        .accel_z_raw_pix          (accel_z_raw_pix),
        
        .accel_update_pix       (accel_update_pix) 
    );
    ...
    stream_led_monitor #(.CLK_HZ(100_000_000),
      .DEC_SHIFT(5),
      .MODE(0),
      .RATE_WINDOW_MS(100),
      .RATE_BYTES_PER_STEP(32)      
    ) u_stream_leds (
        .clk          (clk_100MHz),
        .rst          (rst),
    ...
    tof_7seg_telem_debug #(
        ...
        .dist_mm        (meas_dist_mm),
        .tof_status         (meas_status),
     
     
        .telem_enable (dbg_mode),
    ...
    //==============================================================================
    // 17. Status overlay (right-panel HUD, including ACCEL widget) + rotary bar
    //     FINAL composition chain:
    //        rgb_plot  ->  vga_status_overlay  ->  rotary_bar_overlay  ->  VGA DAC
    //==============================================================================
    
    // -----------------------------------------------------------------------------
    // Pixel-domain reset (synchronize sys reset into pix_clk domain)
    // NOTE: rst is synchronous to clk_100MHz, so treat it as async w.r.t pix_clk.
    // -----------------------------------------------------------------------------
    reg pix_rst_ff1 = 1'b1;
    reg pix_rst_ff2 = 1'b1;
    
    always @(posedge pix_clk) begin
        pix_rst_ff1 <= rst;
        pix_rst_ff2 <= pix_rst_ff1;
    end
    
    wire rst_pix = pix_rst_ff2;
    
    // -----------------------------------------------------------------------------
    // Status overlay: consumes rgb_plot background and draws the right-panel HUD,
    // including ACCEL widget (bars + bubble + digits).
    // -----------------------------------------------------------------------------
    ...
    vga_status_overlay u_status (
        ...
        // Telemetry (system-domain; overlay snapshots internally)
        .sample_q15     (samp_q15),
        ...
        .accel_z_pix    (accel_z_q7_pix),
        
        .accel_x_raw_pix  (accel_x_raw_pix),
        .accel_y_raw_pix  (accel_y_raw_pix),
        .accel_z_raw_pix  (accel_z_raw_pix),
        
        .accel_update   (accel_update_pix),
    ...
    // -----------------------------------------------------------------------------
    // Rotary bar overlay: draws AFTER the HUD, using rgb_overlay as its base.
    // Module performs its own CDC for encoder taps, but still needs a pix reset.
    // -----------------------------------------------------------------------------
