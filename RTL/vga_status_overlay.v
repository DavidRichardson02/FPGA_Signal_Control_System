...
module vga_status_overlay #(
    ...
    // Raw (pix_clk domain)
    input  wire signed [7:0] accel_x_raw_pix,
    input  wire signed [7:0] accel_y_raw_pix,
    input  wire signed [7:0] accel_z_raw_pix,

    input  wire              accel_update,  // 1-cycle pulse when new sample arrives (optional)
...
// ============================================================================
// ACCEL widget local grid (rightmost 160×120 cell, bottom row)
// ============================================================================

localparam integer SCREEN_W       = 640;
localparam integer SCREEN_H       = 480;

localparam integer RIGHT_PANEL_X0 = 280;   // left edge of narrow ACCEL panel
localparam integer RIGHT_PANEL_W  = 160;   // width of ACCEL panel

localparam integer GRID_ROWS      = 4;
localparam integer GRID_COLS      = 2;

localparam integer CELL_Wa        = RIGHT_PANEL_W / GRID_COLS;
localparam integer CELL_Ha        = SCREEN_H      / GRID_ROWS;

// ACCEL widget location: row 3, col 1 (0-based) → bottom-right quadrant
localparam integer ACCEL_ROW_IDX  = 3;
localparam integer ACCEL_COL_IDX  = 1;

localparam integer ACCEL_X0       = RIGHT_PANEL_X0 + ACCEL_COL_IDX * CELL_Wa;
localparam integer ACCEL_Y0       =                  ACCEL_ROW_IDX * CELL_Ha;
localparam integer ACCEL_W        = CELL_Wa;
localparam integer ACCEL_H        = CELL_Ha;

// 3×5 pixel glyphs for X, Y, Z
// char_sel: 0='X', 1='Y', 2='Z'
...
begin
    case (char_sel)
        2'd0: // 'X'
            bits = 15'b101_010_010_010_101;
        2'd1: // 'Y'
            bits = 15'b101_010_010_010_010;
        2'd2: // 'Z'
            bits = 15'b111_001_010_100_111;
        default:
            bits = 15'b000_000_000_000_000;
    endcase
    ...
    //*/
    // Normalize signed Q1.15 turns into an unsigned range [0 .. 0x7FFF] (≈ [0..1) turns)
    function [15:0] q15_turns_norm_u;
        input signed [15:0] th_in;
        reg signed [16:0] tmp;
    begin
        // If negative, wrap by +1.0 turn (0x8000 in Q1.15 signed space)
        tmp = th_in;
        if (tmp < 0)
            tmp = tmp + 17'sd32768;   // +0x8000
    
        // Clamp any pathological values into [0..32767]
        if (tmp < 0) tmp = 0;
        if (tmp > 17'sd32767) tmp = 17'sd32767;
    
        q15_turns_norm_u = tmp[15:0];
    end
    endfunction
    
    // Q1.15 turns -> integer degrees [0..359], rounded and wrapped
    function [8:0] q15_turns_to_deg_0_359;
        input signed [15:0] th_in;
        reg [15:0] th_u;
        reg [31:0] mult;
        reg [15:0] deg_tmp;
    begin
        th_u  = q15_turns_norm_u(th_in);
    
        // degrees ≈ th_u * 360 / 32768; add 16384 for rounding
        mult   = th_u * 32'd360 + 32'd16384;
        deg_tmp = mult >> 15;                 // 0..360 (possible)
    
        // Wrap 360 -> 0
        if (deg_tmp >= 16'd360)
            q15_turns_to_deg_0_359 = 9'd0;
        else
            q15_turns_to_deg_0_359 = deg_tmp[8:0];
    end
    endfunction

    function [6:0] deg_to_pct_0_100;
        input [8:0] deg; // 0..359
        reg [31:0] num;
        reg [15:0] pct;
    begin
        // pct = round(deg * 100 / 360) = (deg*100 + 180)/360
        num = deg * 32'd100 + 32'd180;
        pct = num / 16'd360;
    
        if (pct > 16'd100) pct = 16'd100;
        deg_to_pct_0_100 = pct[6:0];
    end
    endfunction
    
    wire [6:0] theta_pct_0_100 = deg_to_pct_0_100(theta_deg_pix);
    ...
    always @(posedge pix_clk) begin
        if (rst) begin
            ...
            if (frame_tick) begin
                ...
                theta_deg_pix <= q15_turns_to_deg_0_359(theta_s1);
            end
   ...
   // duty_pct_0_100 = round(duty_q15 * 100 / 32768)
    wire [31:0] duty_pct_mult = q15_duty_clamped * 32'd100 + 32'd16384;
    wire [15:0] duty_pct_tmp  = duty_pct_mult >> 15;
    
    wire [6:0] duty_pct_0_100 =
        (duty_pct_tmp > 16'd100) ? 7'd100 : duty_pct_tmp[6:0];
...
// ============================================================================
    // ACCEL widget – grid-aligned cell: signed X/Y/Z bars + bubble level + X/Y/Z
    //             + 3-digit numeric readout (mg in scaled mode) + raw/Q7 debug hook
    // ============================================================================
    
    // ------------------------------
    // Panel hit-test + local coords
    // ------------------------------
    wire in_accel_panel =
    ...
    // ------------------------------------------------------------------------
    // Debug selector: choose what the numeric readout represents
    //   0 => scaled Q7 (accel_*_pix) converted to mg (0..2000 mg with ACC_FULL=64)
    //   1 => raw debug
    // ------------------------------------------------------------------------
    wire signed [7:0] accel_x_scaled = accel_x_pix;
    wire signed [7:0] accel_y_scaled = accel_y_pix;
    wire signed [7:0] accel_z_scaled = accel_z_pix;
    
    wire signed [7:0] accel_x_raw_dbg = accel_x_raw_pix;
    wire signed [7:0] accel_y_raw_dbg = accel_y_raw_pix;
    wire signed [7:0] accel_z_raw_dbg = accel_z_raw_pix;
    
    localparam ACCEL_DEBUG_RAW_DEFAULT = 1'b0;
    wire       accel_debug_raw_mode    = ACCEL_DEBUG_RAW_DEFAULT;
    
    wire signed [7:0] accel_x_dbg_val = accel_debug_raw_mode ? accel_x_raw_dbg : accel_x_scaled;
    wire signed [7:0] accel_y_dbg_val = accel_debug_raw_mode ? accel_y_raw_dbg : accel_y_scaled;
    wire signed [7:0] accel_z_dbg_val = accel_debug_raw_mode ? accel_z_raw_dbg : accel_z_scaled;
    
    // ------------------------------------------------------------------------
    // Frame-stable snapshot (HUD philosophy: coherent per-frame rendering)
    //   1) capture latest on accel_update
    //   2) snapshot to frame on frame_tick
    // Also: frame-stable flash bit so title strip can “blink” when new sample arrived
    // ------------------------------------------------------------------------
    reg signed [7:0] accel_x_latest, accel_y_latest, accel_z_latest;
    reg signed [7:0] accel_x_frame,  accel_y_frame,  accel_z_frame;
    reg              accel_flash_latest;
    reg              accel_flash_frame;
    
    // ------------------------------------------------------------------------
    // ACCEL activity meter (0..255): kicks on accel_update, decays per frame
    // ------------------------------------------------------------------------
    reg [7:0] accel_level;
    always @(posedge pix_clk) begin
        if (rst) begin
            ...
            if (accel_update) begin
                accel_level <= 8'hFF;
            end else if (frame_tick && accel_level != 8'd0) begin
                accel_level <= accel_level - 8'd8;  // faster than UART; tune to taste
            end
        end
    end
    ...
    // ------------------------------------------------------------------------
    // Pitch/Roll estimation (cheap integer approx, degrees)
    //   Assumes ~1g ≈ 32 counts in displayed accel_*_frame units.
    //   deg ≈ acc * (229/128)  (since 57.3/32 ≈ 1.79)
    // ------------------------------------------------------------------------
    reg signed [9:0] roll_deg_frame;   // approx degrees, clamp ±90
    reg signed [9:0] pitch_deg_frame;  // approx degrees, clamp ±90
    
    function signed [9:0] clamp_deg90;
        input signed [15:0] d;
    begin
        if (d > 16'sd90)  clamp_deg90 = 10'sd90;
        else if (d < -16'sd90) clamp_deg90 = -10'sd90;
        else clamp_deg90 = d[9:0];
    end
    endfunction
    
    always @(posedge pix_clk) begin
        if (rst) begin
            roll_deg_frame  <= 10'sd0;
            pitch_deg_frame <= 10'sd0;
        end else if (frame_tick) begin
            // Screen semantics:
            //   roll  ~ +X tilt  (right side down -> positive roll)
            //   pitch ~ -Y tilt  (forward tilt depends on axis; adjust sign as desired)
            // Multiply by 229 then >>>7 is cheap (shift) scaling.
            roll_deg_frame  <= clamp_deg90( ($signed(accel_x_frame) * 16'sd229) >>> 7 );
            pitch_deg_frame <= clamp_deg90( (-$signed(accel_y_frame) * 16'sd229) >>> 7 );
        end
    end
    ...
    always @(posedge pix_clk) begin
        if (rst) begin
            accel_x_latest      <= 8'sd0;
            accel_y_latest      <= 8'sd0;
            accel_z_latest      <= 8'sd0;
            accel_x_frame       <= 8'sd0;
            accel_y_frame       <= 8'sd0;
            accel_z_frame       <= 8'sd0;
            accel_flash_latest  <= 1'b0;
            accel_flash_frame   <= 1'b0;
        end else begin
            if (accel_update) begin
                accel_x_latest     <= accel_x_dbg_val;
                accel_y_latest     <= accel_y_dbg_val;
                accel_z_latest     <= accel_z_dbg_val;
                accel_flash_latest <= 1'b1; // latch until next frame_tick
            end
            if (frame_tick) begin
                accel_x_frame      <= accel_x_latest;
                accel_y_frame      <= accel_y_latest;
                accel_z_frame      <= accel_z_latest;
                accel_flash_frame  <= accel_flash_latest;
                accel_flash_latest <= 1'b0;
            end
        end
    end
    ...
    // ------------------------------------------------------------------------
    // Signed bars (zero-centered): show direction (+/-) and magnitude
    // ------------------------------------------------------------------------
    localparam signed [7:0] ACC_FULL = 8'sd64; // ±64 ≈ ±2g (convention)
    ...
    localparam integer ACCEL_BAR_LEFT   = 8;
    localparam integer ACCEL_BAR_RIGHT  = ACCEL_W - 8;
    localparam integer ACCEL_BAR_W      = ACCEL_BAR_RIGHT - ACCEL_BAR_LEFT;
    localparam integer ACCEL_ZERO_X     = ACCEL_BAR_LEFT + (ACCEL_BAR_W/2);
    
    function signed [9:0] accel_signed_to_dx;
        input signed [7:0] v;
        reg signed [8:0] vv;
    begin
        vv = v;
        if (vv >  ACC_FULL) vv =  ACC_FULL;
        if (vv < -ACC_FULL) vv = -ACC_FULL;
        // map ±ACC_FULL → ±(bar_w/2)
        accel_signed_to_dx = (vv * (ACCEL_BAR_W/2)) / ACC_FULL;
    end
    endfunction
    
    wire signed [9:0] dx_x = accel_signed_to_dx(accel_x_frame);
    wire signed [9:0] dx_y = accel_signed_to_dx(accel_y_frame);
    wire signed [9:0] dx_z = accel_signed_to_dx(accel_z_frame);
    
    wire [9:0] x0_x = (dx_x >= 0) ? ACCEL_ZERO_X : (ACCEL_ZERO_X + dx_x);
    wire [9:0] x1_x = (dx_x >= 0) ? (ACCEL_ZERO_X + dx_x) : ACCEL_ZERO_X;
    
    wire [9:0] x0_y = (dx_y >= 0) ? ACCEL_ZERO_X : (ACCEL_ZERO_X + dx_y);
    wire [9:0] x1_y = (dx_y >= 0) ? (ACCEL_ZERO_X + dx_y) : ACCEL_ZERO_X;
    
    wire [9:0] x0_z = (dx_z >= 0) ? ACCEL_ZERO_X : (ACCEL_ZERO_X + dx_z);
    wire [9:0] x1_z = (dx_z >= 0) ? (ACCEL_ZERO_X + dx_z) : ACCEL_ZERO_X;
    
    wire in_bar_x_row = (accel_y_local >= ACCEL_BAR_TOP_X) && (accel_y_local < ACCEL_BAR_TOP_X + ACCEL_BAR_H);
    wire in_bar_y_row = (accel_y_local >= ACCEL_BAR_TOP_Y) && (accel_y_local < ACCEL_BAR_TOP_Y + ACCEL_BAR_H);
    wire in_bar_z_row = (accel_y_local >= ACCEL_BAR_TOP_Z) && (accel_y_local < ACCEL_BAR_TOP_Z + ACCEL_BAR_H);
    
    wire in_bar_baseline =
        (in_bar_x_row || in_bar_y_row || in_bar_z_row) &&
        (accel_x_local >= ACCEL_BAR_LEFT) &&
        (accel_x_local <  ACCEL_BAR_RIGHT);
    
    wire in_zero_line =
        (accel_x_local >= (ACCEL_ZERO_X-1)) && (accel_x_local <= ACCEL_ZERO_X) &&
        (in_bar_x_row || in_bar_y_row || in_bar_z_row);
    
    wire in_bar_x = in_bar_x_row && (accel_x_local >= x0_x) && (accel_x_local < x1_x);
    wire in_bar_y = in_bar_y_row && (accel_x_local >= x0_y) && (accel_x_local < x1_y);
    wire in_bar_z = in_bar_z_row && (accel_x_local >= x0_z) && (accel_x_local < x1_z);
    
    
    
    
    // ------------------------------------------------------------------------
    // Tiny X/Y/Z glyphs at bar left (3×5 local font)
    // ------------------------------------------------------------------------
    ...
    wire x_glyph_pixel =
        in_x_glyph_box &&
        glyph3x5_pixel(2'd0, x_glyph_gx, x_glyph_gy); // 'X'
    ...
    wire y_glyph_pixel =
        in_y_glyph_box &&
        glyph3x5_pixel(2'd1, y_glyph_gx, y_glyph_gy); // 'Y'
    ...
    wire z_glyph_pixel =
        in_z_glyph_box &&
        glyph3x5_pixel(2'd2, z_glyph_gx, z_glyph_gy); // 'Z'
    
    // ------------------------------------------------------------------------
    // Bubble-level (frame-stable): clamp dot within ring
    // ------------------------------------------------------------------------
    ...
    begin
        if (v < lo) clamp10 = lo;
        else if (v > hi) clamp10 = hi;
        else clamp10 = v;
    end
    endfunction
    
    wire signed [9:0] bubble_dx = clamp10(accel_x_frame >>> 1, -(BUBBLE_R-2), (BUBBLE_R-2));
    wire signed [9:0] bubble_dy = clamp10(accel_y_frame >>> 1, -(BUBBLE_R-2), (BUBBLE_R-2));
    ...
    // ------------------------------------------------------------------------
    // Tilt arrow overlay inside bubble ring
    //   Use vector from center to dot direction.
    //   Coordinate convention here is screen-local:
    //     +x right, +y down.
    //   Dot uses: dot_cx = center + bubble_dx
    //                  dot_cy = center - bubble_dy
    //   So the vector in screen coords is: v = (bubble_dx, -bubble_dy)
    // ------------------------------------------------------------------------
    wire signed [9:0] tilt_vx = bubble_dx;
    wire signed [9:0] tilt_vy = -bubble_dy;
    
    // Pixel relative to center (already have rel_x, rel_y as signed [10:0])
    // Re-use: rel_x = accel_x_local - BUBBLE_CENTER_X
    //         rel_y = accel_y_local - BUBBLE_CENTER_Y
    ...
    wire [9:0] tilt_ax = abs10(tilt_vx);
    wire [9:0] tilt_ay = abs10(tilt_vy);
    wire [10:0] tilt_L  = {1'b0,tilt_ax} + {1'b0,tilt_ay}; // Manhattan length
    
    // Cross / dot tests for "pixel near segment" (thick-ish line)
    wire signed [21:0] cross_val =
        $signed(rel_x) * $signed(tilt_vy) - $signed(rel_y) * $signed(tilt_vx);
    wire [21:0] cross_abs = cross_val[21] ? (~cross_val + 22'd1) : cross_val;
    
    wire signed [21:0] dot_val =
        $signed(rel_x) * $signed(tilt_vx) + $signed(rel_y) * $signed(tilt_vy);
    
    wire [21:0] vv_len2 =
        $signed(tilt_vx) * $signed(tilt_vx) + $signed(tilt_vy) * $signed(tilt_vy);
    
    // Thickness: allow cross error proportional to length (cheap)
    wire [21:0] cross_thresh = {11'd0, tilt_L} << 2; // tune: <<1..<<3
    
    wire tilt_has_vec = (tilt_L > 11'd2);
    
    // Inside segment if dot between [0, |v|^2]
    wire on_segment =
        tilt_has_vec &&
        (dot_val >= 0) &&
        ($unsigned(dot_val) <= vv_len2);
    
    // Near the line if cross small
    wire near_line =
        tilt_has_vec &&
        (cross_abs <= cross_thresh);
    
    // Final line pixel (also keep it inside the bubble disk so it never draws outside)
    wire in_bubble_disk = (dist2_center <= bubble_r2); // already have dist2_center, bubble_r2
    wire in_tilt_line   = in_bubble_disk && on_segment && near_line;
    
    // ------------------------------------------------------------------------
    // Arrowhead: two "wings" behind endpoint, using sign-only unit vectors
    // ------------------------------------------------------------------------
    wire signed [2:0] ux = (tilt_vx == 0) ? 3'sd0 : (tilt_vx[9] ? -3'sd1 : 3'sd1);
    wire signed [2:0] uy = (tilt_vy == 0) ? 3'sd0 : (tilt_vy[9] ? -3'sd1 : 3'sd1);
    
    // perp unit (sign-only)
    wire signed [2:0] pxu = -uy;
    wire signed [2:0] pyu =  ux;
    
    // endpoint in rel-coords is (tilt_vx, tilt_vy)
    wire signed [10:0] wing1_x = $signed(tilt_vx) - $signed(ux*3'sd2) + $signed(pxu);
    wire signed [10:0] wing1_y = $signed(tilt_vy) - $signed(uy*3'sd2) + $signed(pyu);
    
    wire signed [10:0] wing2_x = $signed(tilt_vx) - $signed(ux*3'sd2) - $signed(pxu);
    wire signed [10:0] wing2_y = $signed(tilt_vy) - $signed(uy*3'sd2) - $signed(pyu);
    
    // Compare current rel pixel to wing pixels (1px each)
    wire wing1_px = tilt_has_vec && (rel_x == wing1_x) && (rel_y == wing1_y);
    wire wing2_px = tilt_has_vec && (rel_x == wing2_x) && (rel_y == wing2_y);
    
    // Also draw a tiny endpoint bright pixel (separate from bubble dot)
    wire end_px   = tilt_has_vec && (rel_x == $signed(tilt_vx)) && (rel_y == $signed(tilt_vy));
    
    wire in_tilt_arrowhead = in_bubble_disk && (end_px || wing1_px || wing2_px);
    ...
    // ------------------------------------------------------------------------
    // Numeric readouts:
    //   - scaled mode: mg (0..2000 mg, saturate to 999 for 3 digits)
    //   - raw mode   : raw magnitude (0..255, until true raw path added)
    // ------------------------------------------------------------------------
    function [7:0] abs8;
        input signed [7:0] v;
    begin
    ...
    wire [7:0] ax_abs = abs8(accel_x_frame);
    wire [7:0] ay_abs = abs8(accel_y_frame);
    wire [7:0] az_abs = abs8(accel_z_frame);
    
    // mg = abs * 2000 / 64  (rounded)
    wire [17:0] ax_mg_num = ax_abs * 11'd2000;
    wire [17:0] ay_mg_num = ay_abs * 11'd2000;
    wire [17:0] az_mg_num = az_abs * 11'd2000;
    
    wire [10:0] ax_mg = (ax_mg_num + 18'd32) >> 6;
    wire [10:0] ay_mg = (ay_mg_num + 18'd32) >> 6;
    wire [10:0] az_mg = (az_mg_num + 18'd32) >> 6;
    
    wire [10:0] x_num_val = accel_debug_raw_mode ? {3'd0, ax_abs} : ax_mg;
    wire [10:0] y_num_val = accel_debug_raw_mode ? {3'd0, ay_abs} : ay_mg;
    wire [10:0] z_num_val = accel_debug_raw_mode ? {3'd0, az_abs} : az_mg;
    
    localparam integer ACC_NUM_W   = 3*DIG_W + 2*DIG_SP;
    localparam integer ACC_NUM_X_L = ACCEL_W - ACC_NUM_W - 4;
    
    localparam integer ACC_X_NUM_Y_TOP = 2;
    localparam integer ACC_Y_NUM_Y_TOP = ACC_X_NUM_Y_TOP + (DIG_H + 2);
    localparam integer ACC_Z_NUM_Y_TOP = ACC_Y_NUM_Y_TOP + (DIG_H + 2);
    
    reg [3:0] accx_d2, accx_d1, accx_d0;
    reg [3:0] accy_d2, accy_d1, accy_d0;
    reg [3:0] accz_d2, accz_d1, accz_d0;
    
    integer tmp;
    
    always @* begin
        tmp = x_num_val; if (tmp > 999) tmp = 999;
        accx_d2 = tmp / 100; tmp = tmp % 100;
        accx_d1 = tmp / 10;  accx_d0 = tmp % 10;
    
        tmp = y_num_val; if (tmp > 999) tmp = 999;
        accy_d2 = tmp / 100; tmp = tmp % 100;
        accy_d1 = tmp / 10;  accy_d0 = tmp % 10;
    
        tmp = z_num_val; if (tmp > 999) tmp = 999;
        accz_d2 = tmp / 100; tmp = tmp % 100;
        accz_d1 = tmp / 10;  accz_d0 = tmp % 10;
    end
    
    // Sign marker (a dash) just left of digits when negative
    localparam integer SIGN_W = 5;
    localparam integer SIGN_X = ACC_NUM_X_L - (SIGN_W + 2);
    
    wire in_x_sign =
        accel_x_frame[7] &&
        (accel_x_local >= SIGN_X) && (accel_x_local < SIGN_X + SIGN_W) &&
        (accel_y_local == (ACC_X_NUM_Y_TOP + (DIG_H/2)));
    
    wire in_y_sign =
        accel_y_frame[7] &&
        (accel_x_local >= SIGN_X) && (accel_x_local < SIGN_X + SIGN_W) &&
        (accel_y_local == (ACC_Y_NUM_Y_TOP + (DIG_H/2)));
    
    wire in_z_sign =
        accel_z_frame[7] &&
        (accel_x_local >= SIGN_X) && (accel_x_local < SIGN_X + SIGN_W) &&
        (accel_y_local == (ACC_Z_NUM_Y_TOP + (DIG_H/2)));
    
    // Digit pixels
    reg accel_x_digit_px_on, accel_y_digit_px_on, accel_z_digit_px_on;
    
    integer acc_x_xw, acc_x_row, acc_x_digit_idx, acc_x_col_in_digit;
    integer acc_y_xw, acc_y_row, acc_y_digit_idx, acc_y_col_in_digit;
    integer acc_z_xw, acc_z_row, acc_z_digit_idx, acc_z_col_in_digit;
    
    always @* begin
        accel_x_digit_px_on = 1'b0;
        if ((accel_x_local >= ACC_NUM_X_L) &&
            (accel_x_local <  ACC_NUM_X_L + ACC_NUM_W) &&
            (accel_y_local >= ACC_X_NUM_Y_TOP) &&
            (accel_y_local <  ACC_X_NUM_Y_TOP + DIG_H)) begin
    
            acc_x_row          = accel_y_local - ACC_X_NUM_Y_TOP;
            acc_x_xw           = accel_x_local - ACC_NUM_X_L;
            acc_x_digit_idx    = acc_x_xw / (DIG_W + DIG_SP);
            acc_x_col_in_digit = acc_x_xw % (DIG_W + DIG_SP);
    
            if (acc_x_col_in_digit < DIG_W && acc_x_digit_idx >= 0 && acc_x_digit_idx < 3) begin
                case (acc_x_digit_idx)
                    0: accel_x_digit_px_on = digit_pixel_on(accx_d2, acc_x_row[2:0], acc_x_col_in_digit[2:0]);
                    1: accel_x_digit_px_on = digit_pixel_on(accx_d1, acc_x_row[2:0], acc_x_col_in_digit[2:0]);
                    2: accel_x_digit_px_on = digit_pixel_on(accx_d0, acc_x_row[2:0], acc_x_col_in_digit[2:0]);
                    default: accel_x_digit_px_on = 1'b0;
                endcase
            end
        end
    end
    ...
    always @* begin
        accel_y_digit_px_on = 1'b0;
        if ((accel_x_local >= ACC_NUM_X_L) &&
            (accel_x_local <  ACC_NUM_X_L + ACC_NUM_W) &&
            (accel_y_local >= ACC_Y_NUM_Y_TOP) &&
            (accel_y_local <  ACC_Y_NUM_Y_TOP + DIG_H)) begin
    
            acc_y_row          = accel_y_local - ACC_Y_NUM_Y_TOP;
            acc_y_xw           = accel_x_local - ACC_NUM_X_L;
            acc_y_digit_idx    = acc_y_xw / (DIG_W + DIG_SP);
            acc_y_col_in_digit = acc_y_xw % (DIG_W + DIG_SP);
    
            if (acc_y_col_in_digit < DIG_W && acc_y_digit_idx >= 0 && acc_y_digit_idx < 3) begin
                case (acc_y_digit_idx)
                    0: accel_y_digit_px_on = digit_pixel_on(accy_d2, acc_y_row[2:0], acc_y_col_in_digit[2:0]);
                    1: accel_y_digit_px_on = digit_pixel_on(accy_d1, acc_y_row[2:0], acc_y_col_in_digit[2:0]);
                    2: accel_y_digit_px_on = digit_pixel_on(accy_d0, acc_y_row[2:0], acc_y_col_in_digit[2:0]);
                    default: accel_y_digit_px_on = 1'b0;
                endcase
            end
        end
    end
    ...
    always @* begin
        accel_z_digit_px_on = 1'b0;
        if ((accel_x_local >= ACC_NUM_X_L) &&
            (accel_x_local <  ACC_NUM_X_L + ACC_NUM_W) &&
            (accel_y_local >= ACC_Z_NUM_Y_TOP) &&
            (accel_y_local <  ACC_Z_NUM_Y_TOP + DIG_H)) begin
    
            acc_z_row          = accel_y_local - ACC_Z_NUM_Y_TOP;
            acc_z_xw           = accel_x_local - ACC_NUM_X_L;
            acc_z_digit_idx    = acc_z_xw / (DIG_W + DIG_SP);
            acc_z_col_in_digit = acc_z_xw % (DIG_W + DIG_SP);
    
            if (acc_z_col_in_digit < DIG_W && acc_z_digit_idx >= 0 && acc_z_digit_idx < 3) begin
                case (acc_z_digit_idx)
                    0: accel_z_digit_px_on = digit_pixel_on(accz_d2, acc_z_row[2:0], acc_z_col_in_digit[2:0]);
                    1: accel_z_digit_px_on = digit_pixel_on(accz_d1, acc_z_row[2:0], acc_z_col_in_digit[2:0]);
                    2: accel_z_digit_px_on = digit_pixel_on(accz_d0, acc_z_row[2:0], acc_z_col_in_digit[2:0]);
                    default: accel_z_digit_px_on = 1'b0;
                endcase
            end
        end
    end
    ...
    // ------------------------------------------------------------------------
    // Title strip – tinted differently in raw vs scaled debug modes
    // ------------------------------------------------------------------------
    localparam integer ACCEL_TITLE_H = 12;
    // Header text positions (local coordinates)
    localparam integer ACC_HDR_Y  = 2;     // within title strip
    localparam integer ACC_HDR_X  = 6;     // left padding
    localparam integer ACC_MODE_X = 34;    // to the right of "ACC"
    wire in_title_strip = (accel_y_local < ACCEL_TITLE_H);
    
    // ------------------------------------------------------------------------
    // Color generation for ACCEL
    // ------------------------------------------------------------------------
    reg  [3:0] accel_r, accel_g, accel_b;
    wire       accel_draw = in_accel_panel;
    ...
    always @* begin
        ...
        if (in_accel_panel) begin
            ...
            // Title strip indicates mode + flashes on new sample
            if (in_title_strip) begin
                if (accel_debug_raw_mode) begin
                    accel_r = 4'h8; accel_g = 4'h2; accel_b = 4'h8;
                end else begin
                    accel_r = 4'h4; accel_g = 4'h4; accel_b = 4'h8;
                end
                if (accel_flash_frame) begin
                    accel_r = 4'hF; accel_g = 4'hF; accel_b = 4'hF;
                end
            end
    
            // Baseline + zero line
            if (in_bar_baseline) begin
                accel_r = 4'h3; accel_g = 4'h3; accel_b = 4'h5;
            end
            if (in_zero_line) begin
                accel_r = 4'hC; accel_g = 4'hC; accel_b = 4'hC;
            end
    
            // Bars (positive brighter, negative darker)
            if (in_bar_x) begin
                if (accel_x_frame[7]) begin accel_r=4'hA; accel_g=4'h2; accel_b=4'h2; end
                else                  begin accel_r=4'hF; accel_g=4'h3; accel_b=4'h3; end
            end
            if (in_bar_y) begin
                if (accel_y_frame[7]) begin accel_r=4'h2; accel_g=4'hA; accel_b=4'h2; end
                else                  begin accel_r=4'h3; accel_g=4'hF; accel_b=4'h3; end
            end
            if (in_bar_z) begin
                if (accel_z_frame[7]) begin accel_r=4'h2; accel_g=4'h3; accel_b=4'hA; end
                else                  begin accel_r=4'h3; accel_g=4'h4; accel_b=4'hF; end
            end
            ...
            if (in_bubble_dot) begin
                accel_r = 4'hF; accel_g = 4'hF; accel_b = 4'h0;
            end
    
            // Glyphs
            if (x_glyph_pixel) begin accel_r=4'hF; accel_g=4'h6; accel_b=4'h6; end
            if (y_glyph_pixel) begin accel_r=4'h6; accel_g=4'hF; accel_b=4'h6; end
            if (z_glyph_pixel) begin accel_r=4'h6; accel_g=4'h6; accel_b=4'hF; end
    
            // Digits + sign markers
            if (accel_x_digit_px_on || accel_y_digit_px_on || accel_z_digit_px_on ||
                in_x_sign || in_y_sign || in_z_sign) begin
                accel_r = 4'hF; accel_g = 4'hF; accel_b = 4'hF;
            end
        end
    end
  
    localparam integer ACC_BAR_Y  = 9;     // inside title strip
    localparam integer ACC_BAR_X0 = ACCEL_W - 52;
    localparam integer ACC_BAR_X1 = ACCEL_W - 6;
    
    wire in_acc_act_bar_y = in_title_strip && (accel_y_local == ACC_BAR_Y);
    wire [9:0] acc_act_x_r = ACC_BAR_X0 + {2'b00, accel_level[7:3]}; // 0..31 px
    
    wire in_acc_act_bar =
        in_acc_act_bar_y &&
        (accel_x_local >= ACC_BAR_X0) &&
        (accel_x_local <  ACC_BAR_X1) &&
        (accel_x_local <= acc_act_x_r);
    ...
    wire acc_A_px, acc_C_px, acc_C2_px;
        vga_char_glyph u_acc_A (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_HDR_X + 10'd0),  .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("A"), .pixel_on(acc_A_px)
        );
        vga_char_glyph u_acc_C1 (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_HDR_X + 10'd8),  .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("C"), .pixel_on(acc_C_px)
        );
        vga_char_glyph u_acc_C2 (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_HDR_X + 10'd16), .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("C"), .pixel_on(acc_C2_px)
        );
        
        wire acc_mode_R_px, acc_mode_A_px, acc_mode_W_px;
        wire acc_mode_Q_px, acc_mode_7_px;
        
        vga_char_glyph u_mode_Q (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_MODE_X + 10'd0), .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("Q"), .pixel_on(acc_mode_Q_px)
        );
        vga_char_glyph u_mode_7 (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_MODE_X + 10'd8), .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("7"), .pixel_on(acc_mode_7_px)
        );
        
        vga_char_glyph u_mode_R (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_MODE_X + 10'd0), .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("R"), .pixel_on(acc_mode_R_px)
        );
        vga_char_glyph u_mode_A (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_MODE_X + 10'd8), .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("A"), .pixel_on(acc_mode_A_px)
        );
        vga_char_glyph u_mode_W (
            .clk_pix(pix_clk), .rst_pix(rst),
            .hcount(hcount), .vcount(vcount), .active_video(active_video),
            .x0(ACCEL_X0 + ACC_MODE_X + 10'd16), .y0(ACCEL_Y0 + ACC_HDR_Y),
            .char_code("W"), .pixel_on(acc_mode_W_px)
        );
        
        wire acc_hdr_text_on =
            acc_A_px | acc_C_px | acc_C2_px |
            (accel_debug_raw_mode ? (acc_mode_R_px | acc_mode_A_px | acc_mode_W_px)
                                  : (acc_mode_Q_px | acc_mode_7_px));
    ...
    always @* begin
        if (!active_video) begin
            ...
            // ---- ACCEL widget (priority within its grid cell) ----
            if (accel_draw) begin
                rgb_out[11:8] = accel_r;
                rgb_out[7:4]  = accel_g;
                rgb_out[3:0]  = accel_b;
            end   
            
            // Activity bar overrides accel background (still inside accel cell)
            if (accel_draw && in_acc_act_bar) begin
                rgb_out = 12'hFF0; // yellow
            end
            
            // Header text overrides everything in accel title strip
            if (accel_draw && acc_hdr_text_on) begin
                rgb_out = 12'hFFF; // white
            end            // Units text
            //if (acc_units_on) begin
            //    accel_r = 4'hF; accel_g = 4'hF; accel_b = 4'hF;
            //end

            // Tilt arrow (line + arrowhead) inside bubble ring
            if (accel_draw && in_tilt_line) begin
                rgb_out = 12'h0FF; // cyan line
            end
            if (accel_draw && in_tilt_arrowhead) begin
                rgb_out = 12'hFF0; // yellow arrowhead
            end
