...
// ============================================================================
//
// NOTE (Persistence upgrade):
// ----------------------------------------------------------------------------
// This emitted version preserves original comments above, but updates the
// framebuffer from 1-bit to 4-bit intensity and replaces "hard clear" with
// "decay sweep" once per sweep_wrap. This creates decay trails / persistence.
//
// Required companion modules (provided previously):
//   - tof_plot_bram_dp4          (256x256, 4-bit, dual-port, Port A readback)
//   - tof_frame_decay_fsm        (read-modify-write sweep: I <- max(I-1,0))
//   - tof_plot_point_writer_int4 (writes HIT_LEVEL on each ToF sample)
//
// ============================================================================
...
module vga_range_plot_top #(
    ...
    reg [1:0] rst_pix_ff = 2'b11;   // init helps simulation a lot
    always @(posedge pix_clk) begin
        if (rst_sys) begin
            rst_pix_ff <= 2'b11;    // hold reset asserted while rst_sys=1
        end else begin
            rst_pix_ff <= {rst_pix_ff[0], 1'b0};
        end
    end
    ...
    //
    // PERSISTENCE UPDATE:
    //   - This framebuffer is now treated as a 4-bit intensity plane, not 1-bit.
    //   - The old clear FSM is replaced with a decay FSM:
    //         I <- max(I - DECAY_STEP, 0)
    //   - The plot writer writes HIT_LEVEL (typically 15) on each hit.
    //
    // Rationale:
    //   - 1-bit storage can only show ON/OFF; persistence requires multiple
    //     levels so pixels can fade gradually.
    // ========================================================================

    // Port A (clk_100mhz domain) is written by:
    //   * tof_frame_decay_fsm       : decays all pixels once per sweep.
    //   * tof_plot_point_writer_int4: stamps ToF samples at high intensity.
    wire        plot_wr_en;
    wire [7:0]  plot_wr_x;
    wire [7:0]  plot_wr_y;
    wire [3:0]  plot_wr_data;

    wire        decay_busy;
    wire        decay_wr_en;
    wire [7:0]  decay_wr_x;
    wire [7:0]  decay_wr_y;
    wire [3:0]  decay_wr_data;



    // Plot writer (additive persistence / accumulation)
    wire [7:0] plot_a_x;
    wire [7:0] plot_a_y;
    wire       plot_a_we;
    wire [3:0] plot_a_d;
    
    // Write-port arbitration (Port A):
    //   - When decay_busy=1, decay FSM owns the write port.
    //   - Otherwise, plot writer owns the write port.
    // --------------------------------------------------------------------
    // Port A ownership:
    //   - decay_busy=1  -> decay FSM drives address and writes decayed data
    //   - decay_busy=0  -> plot writer drives address and performs RMW
    // --------------------------------------------------------------------
    wire [7:0] fb_a_x     = decay_busy ? decay_wr_x    : plot_a_x;
    wire [7:0] fb_a_y     = decay_busy ? decay_wr_y    : plot_a_y;
    wire       fb_a_we    = decay_busy ? decay_wr_en   : plot_a_we;
    wire [3:0] fb_a_d     = decay_busy ? decay_wr_data : plot_a_d;


    // Port B (pix_clk): read side (used by the colorizer).
    reg  [7:0] fb_rd_x;
    reg  [7:0] fb_rd_y;
    wire [3:0] fb_rd_q4;      // 4-bit intensity pixel from framebuffer

    // Port A readback (clk_100mhz domain), required for decay RMW
    wire [3:0] fb_rd_q4_a;

    // Dual-port BRAM (intensity version)
    tof_plot_bram_dp4 u_tof_fb (
        // Port A: system domain (read/write for decay)
        .clk_a (clk_100mhz),
        .we_a  (fb_a_we),
        .x_a   (fb_a_x),
        .y_a   (fb_a_y),
    ...
    //
    // PERSISTENCE UPDATE:
    //   - Instead of clearing to 0, we DECAY intensity each sweep:
    //         I <- max(I - 1, 0)
    //   - This gives "trails" that fade smoothly over multiple sweeps.
    // ========================================================================

    tof_frame_decay_fsm #(
        .ADDR_W     (16),
        .DECAY_STEP (4'd1)      // tune fade speed: 1 slow, 2..4 faster
    ) u_frame_decay (
        .clk_sys     (clk_100mhz),
        .rst_sys     (rst_sys),
        .start_decay (sweep_wrap),
        .busy        (decay_busy),
        .wr_en       (decay_wr_en),
        .wr_x        (decay_wr_x),
        .wr_y        (decay_wr_y),
        .wr_data     (decay_wr_data),
        .rd_data_a   (fb_rd_q4_a)
    );
    ...
    //
    // PERSISTENCE UPDATE:
    //   - Writer now stamps a 4-bit intensity value (HIT_LEVEL) rather than a
    //     single bit. New hits appear bright, then fade via decay sweep.
    //   - Writes are inhibited while decay_busy to avoid Port A conflicts.
    // ========================================================================
    //
    /*
    tof_plot_point_writer_int4 #(
        .R_MAX_MM   (R_MAX_MM),
        .HIT_LEVEL  (4'hF)
    ) u_plot_writer (
        .clk_sys    (clk_100mhz),
        .rst_sys    (rst_sys),
        .decay_busy (decay_busy),
        .dist_mm    (dist_mm),
        .dist_vld   (dist_vld),
        .theta_q15  (theta_q15),
        .wr_en      (plot_wr_en),
    ...
    tof_plot_point_writer_accum_fsm #(
        .R_MAX_MM (R_MAX_MM),
        .HIT_INC  (4'd3)     // tune: 1..6 typical
    ) u_plot_writer_accum (
        .clk_sys    (clk_100mhz),
        .rst_sys    (rst_sys),
        .decay_busy (decay_busy),
    ...
    //   Framebuffer is 256 columns wide, but plot region is 320 pixels
    //   wide. The old mapping:
    //       fb_rd_x <= hcount[8:1];   // 0..319 → 0..159
    //   only displayed the left 160 columns of the 256-wide framebuffer.
    //
    //   We now map 0..319 → 0..255 using:
    //       x_fb = floor(x_screen * 256 / 320) = floor(x_screen * 4 / 5)
    //
    //   This ensures the full framebuffer width is visible on screen and that
    //   theta_q15[15:8] (0..255) aligns with what see.
    // ========================================================================

    localparam integer PLOT_X0 = 0;
    localparam integer PLOT_X1 = 319;
    localparam integer PLOT_Y0 = LOGO_Y1 + 1;  // 120
    //localparam integer PLOT_Y1 = 10'd119 + 255;   // 256 rows below PLOT_Y0
    localparam integer PLOT_Y1 = PLOT_Y0 + 255; // 375
    ...
    wire [9:0]  x_screen = hcount - PLOT_X0;     // 0..319 inside plot box
    wire [11:0] x_mul4   = {x_screen, 2'b00};    // x_screen * 4
    wire [7:0]  x_fb_map = x_mul4 / 10'd5;       // divide by 5 → 0..255
    ...
    // Recover current framebuffer X index from the corrected mapping
    wire [7:0] x_fb_current = x_fb_map;
    ...
    //
    // PERSISTENCE UPDATE:
    //   - Instead of a 1-bit hit, fb_rd_q4 is a 4-bit intensity.
    //   - We map intensity to brightness (green channel) so newer hits are
    //     brighter and older hits fade.
    // ========================================================================
    ...
    always @(posedge pix_clk) begin
        if (rst_pix) begin
            ...
            // ----------------------------------------------------------------
                ...
                if (fb_rd_q4 != 4'd0) begin
                    // ToF hit intensity: brightness = fb_rd_q4 (0..15)
                    r_reg <= 4'h0;
                    g_reg <= fb_rd_q4;
                    b_reg <= 4'h0;
                end else begin
    ...
    // Enabling here means cutting off elsewhere./*
    // ========================================================================
    // 9) Simulation-only VGA frame capture (PPM dump)
    // ------------------------------------------------------------------------
    // NOTE:
    //   This block is NOT synthesizable (uses $fopen/$fwrite/$finish).
    //   It is wrapped in `ifndef SYNTHESIS so it exists only in simulation.
    //
    // Output:
    //   vga_frame_000.ppm, vga_frame_001.ppm, ... in the simulator run folder.
    // ========================================================================

    `ifndef SYNTHESIS
        vga_frame_capture_rgb444 #(
            .H_VISIBLE  (640),
            .V_VISIBLE  (480),
            .MAX_FRAMES (3),
            .OUT_PREFIX ("vga_frame_")
        ) u_cap_rgb444 (
            .pix_clk      (pix_clk),
            .rst_pix      (rst_pix),
