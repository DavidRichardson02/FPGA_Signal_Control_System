...
//     [13] duty_q15[7:0]
//
//     [14] accel_x_q7       (signed Q7 accel X sample; 1 LSB = 1/128 g if defined it that way)
//     [15] accel_y_q7       (signed Q7 accel Y sample)
//     [16] accel_z_q7       (signed Q7 accel Z sample)
//     [17] accel_flags      (accelerometer health/valid flags; see bitfield below)
//
//     [18] status           (sensor quality/error bits)
//
//     [19] CRC[15:8]        (CRC16-CCITT over bytes [2..18])
//     [20] CRC[7:0]
//
//   accel_flags bitfield (byte [17]):
//     bit0 = accel_init_done   (1 when ADXL init sequence has completed)
//     bit1 = accel_devid_error (1 when DEVID mismatch/error detected)
//     bit2 = accel_seen        (1 once at least one accel sample has been observed since reset)
//     bit7:3 = reserved (0)
//
//   CRC details:
...
//     • CRC now covers the extended payload bytes [2..18] (t_us, theta, dist,
//       temp_q15, duty_q15, accel_xyz, accel_flags, status).
//
...
//     • Packet length is fixed at 21 bytes.
//
// Timestamp model:
...
//
// Accelerometer capture model:
//   • accel_x_q7/accel_y_q7/accel_z_q7 are *not* required to be aligned to
//     sample_vld. This block maintains "last-known" accel samples whenever
//     accel_sample_vld is asserted, and snapshots those last-known values when
//     a ToF packet begins (sample_vld in P_IDLE).
//
// Implementation notes:
//   • Inputs are snapshotted on 'sample_vld' in P_IDLE to avoid tearing.
//   • A simple FSM walks through the 21 bytes, feeding the CRC on payload
//     bytes [2..18] as they are emitted.
...
module mapper_packetizer #(                         // [opcode:module] [operands: name=mapper_packetizer, has parameters]
    ...
    // NEW: accelerometer (signed Q7) + health flags
    input  wire signed [7:0] accel_x_q7,            // [opcode:input] [operands: width=8, signed Q7 accel X]
    input  wire signed [7:0] accel_y_q7,            // [opcode:input] [operands: width=8, signed Q7 accel Y]
    input  wire signed [7:0] accel_z_q7,            // [opcode:input] [operands: width=8, signed Q7 accel Z]
    input  wire              accel_sample_vld,      // [opcode:input] [operands: strobe=1 cycle, new accel sample]
    input  wire              accel_init_done,       // [opcode:input] [operands: 1 when accel init completed]
    input  wire              accel_devid_error,     // [opcode:input] [operands: 1 when DEVID mismatch/error]
...
// ---------------- Accelerometer "last-known" capture ----------------
// Theory: accel_sample_vld updates the last-known accel sample. Packets snapshot
// these last-known values when a ToF sample begins.
    reg signed [7:0] ax_last;                       // [opcode:reg] [operands: last accel X]
    reg signed [7:0] ay_last;                       // [opcode:reg] [operands: last accel Y]
    reg signed [7:0] az_last;                       // [opcode:reg] [operands: last accel Z]
    reg              accel_seen;                    // [opcode:reg] [operands: seen at least one accel sample]

    always @(posedge clk) begin                     // [opcode:always] [operands:sens=posedge clk]
        if (rst) begin                              // [opcode:if] [operands: reset]
            ax_last    <= 8'sd0;                    // [opcode:nba]
            ay_last    <= 8'sd0;                    // [opcode:nba]
            az_last    <= 8'sd0;                    // [opcode:nba]
            accel_seen <= 1'b0;                     // [opcode:nba]
        end else if (accel_sample_vld) begin        // [opcode:else if] [operands: new accel sample]
            ax_last    <= accel_x_q7;               // [opcode:nba]
            ay_last    <= accel_y_q7;               // [opcode:nba]
            az_last    <= accel_z_q7;               // [opcode:nba]
            accel_seen <= 1'b1;                     // [opcode:nba]
        end
    end
...
// Theory: De-couple incoming busses from stream timing; capture atomically.
    ...
    // NEW: latched accelerometer sample + flags
    reg signed [7:0] ax_latch;                      // [opcode:reg] [operands: latched accel X]
    reg signed [7:0] ay_latch;                      // [opcode:reg] [operands: latched accel Y]
    reg signed [7:0] az_latch;                      // [opcode:reg] [operands: latched accel Z]
    reg [7:0]         af_latch;                     // [opcode:reg] [operands: latched accel_flags byte]
...
// Note: PACK_LEN_BYTES is for documentation / debug.
    localparam integer PACK_LEN_BYTES = 21;         // [opcode:localparam] [operands: constant for docs]
    ...
    localparam [4:0]                                  // [opcode:localparam group] [operands: 5-bit encodings]
        ...
        P_AX     = 5'd15,                             // [opcode:const] [operands: accel_x_q7]
        P_AY     = 5'd16,                             // [opcode:const] [operands: accel_y_q7]
        P_AZ     = 5'd17,                             // [opcode:const] [operands: accel_z_q7]
        P_AF     = 5'd18,                             // [opcode:const] [operands: accel_flags]
        P_S      = 5'd19,                             // [opcode:const] [operands: status]
        P_C1     = 5'd20,                             // [opcode:const] [operands: CRC[15:8]]
        P_C0     = 5'd21;                             // [opcode:const] [operands: CRC[7:0]]
    ...
    always @(posedge clk) begin                      // [opcode:always] [operands:sens=posedge clk]
        ...
        if (rst) begin                               // [opcode:if] [operands: reset path]
            ...
            ax_latch  <= 8'sd0;                      // [opcode:nba]
            ay_latch  <= 8'sd0;                      // [opcode:nba]
            az_latch  <= 8'sd0;                      // [opcode:nba]
            af_latch  <= 8'd0;                       // [opcode:nba]
            ...
            case (ps)                                // [opcode:case] [operands: switch ps]
                ...
                P_IDLE: begin                        // [opcode:label] [operands: state=P_IDLE]
                    if (sample_vld) begin            // [opcode:if] [operands: edge-qualified strobe]
                        ...
                        // Snapshot last-known accelerometer sample (from accel_sample_vld updates)
                        ax_latch   <= ax_last;       // [opcode:nba] [operands: snapshot accel X]
                        ay_latch   <= ay_last;       // [opcode:nba] [operands: snapshot accel Y]
                        az_latch   <= az_last;       // [opcode:nba] [operands: snapshot accel Z]

                        // accel_flags is latched so it matches accel_xyz for this frame
                        af_latch   <= {5'b0, accel_seen, accel_devid_error, accel_init_done}; // [opcode:nba] [operands: pack flags]
                ...
                // ---- Accelerometer XYZ (signed Q7, one byte each) ----
                P_AX: if (tx_rdy) begin
                    tx_byte <= ax_latch[7:0];        // [opcode:nba] [operands: byte[14]]
                    tx_vld  <= 1'b1;
                    crc     <= crc16_ccitt(crc, ax_latch[7:0]);
                    ps      <= P_AY;
                end
                P_AY: if (tx_rdy) begin
                    tx_byte <= ay_latch[7:0];        // [opcode:nba] [operands: byte[15]]
                    tx_vld  <= 1'b1;
                    crc     <= crc16_ccitt(crc, ay_latch[7:0]);
                    ps      <= P_AZ;
                end
                P_AZ: if (tx_rdy) begin
                    tx_byte <= az_latch[7:0];        // [opcode:nba] [operands: byte[16]]
                    tx_vld  <= 1'b1;
                    crc     <= crc16_ccitt(crc, az_latch[7:0]);
                    ps      <= P_AF;
                end

                // ---- accel_flags (fed to CRC) ----
                P_AF: if (tx_rdy) begin
                    tx_byte <= af_latch;             // [opcode:nba] [operands: byte[17]]
                    tx_vld  <= 1'b1;
                    crc     <= crc16_ccitt(crc, af_latch);
                    ps      <= P_S;
                end

                // ---- Status (last payload byte fed to CRC) ----
                P_S: if (tx_rdy) begin
                    tx_byte <= s_latch;              // [opcode:nba] [operands: byte[18]]
                    tx_vld  <= 1'b1;
                ...
                P_C1: if (tx_rdy) begin
                    tx_byte <= crc[15:8];            // [opcode:nba index] [operands: byte[19]]
                    tx_vld  <= 1'b1;
                    ps      <= P_C0;
                end
                P_C0: if (tx_rdy) begin
                    tx_byte <= crc[7:0];             // [opcode:nba index] [operands: byte[20]]
                    tx_vld  <= 1'b1;
